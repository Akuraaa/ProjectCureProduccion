...
public class AutomaticGunScriptLPFP : MonoBehaviour
{
	...
	{
		if (FindObjectOfType<UIManager>().isPaused == true)
		{
			//Weapon sway
			if (weaponSway == true)
			{
	...
	private void Update()
	{
			...
			if (Input.GetKeyDown(KeyCode.V) && !isInspecting)
		{
			...
			if (Input.GetKeyDown(KeyCode.G) && FindObjectOfType<PlayerStats>().haveGrenade && FindObjectOfType<PlayerStats>().grenadeCount > 0)
			{
				FindObjectOfType<PlayerStats>().grenadeCount--;
